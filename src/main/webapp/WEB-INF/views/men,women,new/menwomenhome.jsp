...
<body>
    ...
    <table class="t2">
        ...
        <tr>
          <c:forEach items="${menwomenhome }" var="dto" varStatus ="status">
    ...
    <form action="menwomenhome" method="post">
	<c:if test="${searchVO.totPage>1 }"> <!-- 토탈 페이지가 1보다 크면 -->
		<c:if test="${searchVO.page>1 }">
			<a href="menwomenhome?page=1">[처음]</a>
			<a href="menwomenhome?page=${searchVO.page-1 }">[이전]</a>
		</c:if>
	...
	<c:forEach begin="${searchVO.pageStart }" end="${searchVO.pageEnd }" var="i">
		<c:choose>
			<c:when test="${i eq searchVO.page }">  <!-- i eq searchVO.page 내가 클릭한 페이지가 같을 때 -->
			 ...
			 <c:otherwise>
				<a href="menwomenhome?page=${i }" style="text-decoration: none">${i }</a>&nbsp;
			</c:otherwise> 
		
		</c:choose>
	</c:forEach>
		<c:if test="${searchVO.totPage>searchVO.page }"> <!-- 마지막 페이지가 아닐때 -->
			<a href="menwomenhome?page=${searchVO.page+1 }">></a>
			<a href="menwomenhome?page=${searchVO.totPage }">>></a>
		</c:if>
	</c:if>
    </div>
